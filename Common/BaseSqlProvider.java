...
package com.ncepu.feilong505.OASystem.Common;
...
public class BaseSqlProvider {
	...
	// 自定义的分页查询
	// 重写生成sql，因为object作为一个参数无法直接提供内部参数
		...
		if (object == null)
			...
			try {
				...
				for (Map.Entry<String, String> pair : baseResultMapping.entrySet()) {
					...
					Object value = field.get(object);
					if (value != null) {
						sql.WHERE(String.format("`%s`= #{object.%s}", column, property));
					}
				}
				StringBuilder sqlNew = new StringBuilder(sql.toString());
				sqlNew.append(String.format("LIMIT %s, %s", (pageNum - 1) * pageSize, pageSize));
				ans = sqlNew.toString();
